...
3. For each role (orchestrator, backend, frontend), read `plans/round-{N-1}/summary-{role}.md` if it exists — these are the compacted context from the previous round
...
Each file follows the structure used in `plans/round-1/orchestrator.md`:
...
- `## Wrap-up` — instruction to write `plans/round-N/summary-<role>.md` with sections: **Built**, **Deviations**, **Deferred**, **Next round needs to know**, **Config improvements**
...
## Before writing
Ask the user up to 3 clarifying questions if anything is ambiguous (scope cuts, library choice for a new feature, etc.). Otherwise proceed without questions.
